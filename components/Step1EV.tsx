...
const Step1EV: React.FC<Step1EVProps> = ({ ev, onEvChange, onNext }) => {
  // Find the closest scenario to the current EV
  const currentScenario = SCENARIOS.reduce((prev, curr) => 
    Math.abs(curr.ev - ev) < Math.abs(prev.ev - ev) ? curr : prev
  );
  ...
  useEffect(() => {
    const activeBtn = dialRef.current?.querySelector(`[data-ev="${ev}"]`);
    if (activeBtn) {
  ...
  return (
    <div className="flex-1 flex flex-col">
      ...
      <main className="flex-1 flex flex-col justify-between py-6 px-6 relative">
        <div className="text-center pt-4">
          <div className="inline-flex flex-col items-center">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-7xl font-bold tracking-tighter italic text-white tabular-nums">{ev}</span>
              <span className="text-xl font-bold text-primary italic">EV</span>
            </div>
          </div>
        </div>

        <div className="bg-background-charcoal border-l-4 border-primary p-5 shadow-xl relative overflow-hidden transition-all duration-500 min-h-[140px]">
          <div className="absolute -top-4 -right-4 opacity-[0.05] scale-150 transition-all duration-700">
            <span className="material-symbols-outlined text-9xl">{currentScenario.icon}</span>
          </div>
          <div className="relative z-10">
            <div className="flex items-center gap-4 mb-3">
              <div className="w-12 h-12 flex items-center justify-center bg-white/5 border border-white/10 rounded-sm">
                <span className="material-symbols-outlined text-primary text-2xl transition-all duration-300">{currentScenario.icon}</span>
              </div>
              <div>
                <h3 className="font-bold text-base tracking-wide uppercase transition-all duration-300">{currentScenario.name}</h3>
        ...
        {/* Dial UI - Supporting -6 to 18 */}
        <div className="relative w-full py-10 flex flex-col items-center">
          <div className="absolute left-1/2 top-0 -translate-x-1/2 w-[2px] h-full bg-primary z-20 pointer-events-none">
          ...
          <div ref={dialRef} className="w-full overflow-x-auto hide-scrollbar dial-gradient snap-x snap-mandatory flex px-[45%]">
            <div className="flex items-end gap-6 py-4">
              {evRange.map(val => (
                <button 
                  ...
                  className={`flex flex-col items-center transition-all duration-300 snap-center min-w-[44px] ${ev === val ? 'opacity-100 scale-125' : 'opacity-20 scale-90'}`}
                >
                  <div className={`w-0.5 bg-white mb-2 transition-all ${ev === val ? 'h-14 bg-primary' : 'h-6 bg-white/50'}`}></div>
                  <span className={`text-xs font-bold font-mono tabular-nums ${ev === val ? 'text-primary' : 'text-white'}`}>{val}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="rounded-xl overflow-hidden h-28 border border-white/10 group relative shadow-2xl">
          ...
          <div className="absolute bottom-2 left-3 flex items-center gap-2">
            <div className="w-1.5 h-1.5 bg-primary rounded-full animate-pulse shadow-[0_0_8px_#E30613]"></div>
      ...
      <footer className="p-6 bg-background-charcoal border-t border-white/5">
        <button 
          onClick={onNext}
          className="w-full py-5 bg-primary text-white font-bold text-sm uppercase tracking-widest flex items-center justify-center gap-3 active:brightness-90 transition-all rounded-sm"
        >
          Lock Ambient Light
